...
parser = EVEDParser(staticPath=EVED_STATIC, vehicleTypes=args.vehicle_types)
...
trajectoriesForSUMO = buildTrajectoryDataframe(trajectories, True)

# Run SUMO simulation
runSimulation(trajectoriesForSUMO, args.depart_delay)

# Generate virtual dataset using simulation results
generateVirtualDataset(OUTPUT / "tripinfos.xml", trajectoriesForSUMO)
